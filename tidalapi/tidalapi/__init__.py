...
class Session(object):
    ...
    def login(self, username, password, api_token):
...
#        return self.request('GET', 'artists/%s/bio' % artist_id).json()['text']
        res = self.request('GET', 'artists/%s/bio' % artist_id)
        if res != 404:
            return self.request('GET', 'artists/%s/bio' % artist_id).json()['text']
        else:
            return ""
    ...
    def get_media_url(self, track_id):
        ...
        return r.json()
        #params = {'audioquality': self._config.quality, 'assetpresentation': 'FULL', 'urlusagemode':'STREAM'}
